...
class Checkout extends React.Component {
    ...
    $.ajax({
      ...
      success: results => {
        for (var i = 0; i < results.length; i++) {
          var newState = this.state.reservedDays.concat([
            [results[i].checkin, results[i].checkout]
          ]);
          this.setState({
  ...
  checkOpenings(event, data) {
    ...
    if (this.state.startDate === null || this.state.endDate === null) {
      ...
      for (var j = 0; j < resDates.length; j++) {
        if (
          moment(resDates[j][0], "MM-DD-YYYY").isBetween(
    ...
    if (conflict) {
      ...
      if (this.state.modalOpen === true) {
        $("<div class='warning'>Please select a valid range of dates</div>")
          .prependTo(".modal")
          .fadeOut(1500);
  ...
  makeReservation(reservationInfo) {
    ...
    $.ajax({
      ...
      success: () => {
        ...
        if (this.state.modalOpen === true) {
          $("<div class='warning'>Successfully booked</div>")
            .prependTo(".modal")
            .fadeOut(2000);
        } else if (this.state.modalOpen === false) {
          $("<div class='warning'>Successfully booked</div>")
  ...
  isDayBlocked(day) {
    ...
    for (var i = 0; i < resDates.length; i++) {
      if (
        day.isBetween(
          moment(resDates[i][0], "MM-DD-YYYY"),
          moment(resDates[i][1], "MM-DD-YYYY"),
          "days",
          "[]"
        )
      ) {
          ...
          {matches =>
            matches ? (
              <div>
                  ...
                  <DateRangePicker
                    ...
                    renderCalendarInfo={() => {
                      return (
                        <div className="info">
                          <div>
                            {this.state.minNights + " night(s) minimum"}
                          </div>
                ...
                <div>
                  <FormBot
                    checkOpenings={this.checkOpenings.bind(this)}
                    details={this.state}
                  />
                </div>
              </div>
              ...
              <div>
                <div>
                  <div className="footer">
                    <Header info={this.state} />
                    <button className="sub-but" onClick={this.openModal}>
                      BOOK
                    </button>
                  </div>
                  ...
                  >
                    <button className="close-but" onClick={this.closeModal}>
                      X
                    </button>
                        ...
                        renderCalendarInfo={() => {
                          return (
                            <div className="info">
                              ...
                              <div>
                                {this.state.maxGuests + " guests allowed"}
                              </div>
                            </div>
                          );
                        }}
